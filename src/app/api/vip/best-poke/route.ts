...
// do combate. So SUGERE (o jogo nao expoe trocar o pokemon ativo; voce troca no jogo).
export async function GET(req: Request) {
  ...
  let best: { speciesId: number; name: string; level: number; power: number; eff: number; score: number } | null = null;
  for (const p of team) {
    ...
    if (!best || score > best.score) best = { speciesId: p.speciesId, name: p.name, level: p.level, power: p.power, eff, score };
  }
  if (!best) return NextResponse.json({ best: null });
  return NextResponse.json({
    best: { speciesId: best.speciesId, name: best.name, level: best.level, power: best.power, eff: Math.round(best.eff * 100) / 100 },
