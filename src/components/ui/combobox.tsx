  ...
  return (
    <>
      ...
      >
        ...
        {open ? (
          <input
            ...
            onKeyDown={onKey}
            className="min-w-0 flex-1 bg-transparent text-[12px] text-text outline-none placeholder:text-text-mute"
          />
        ) : (
          <span className={cn("min-w-0 flex-1 truncate text-[12px]", !selected && "text-text-mute")}>
      ...
      <Popover open={open} onClose={close} anchorRef={anchor} matchWidth maxHeight={340}>
        <PopoverScroll>
          {hits.length === 0 ? (
            <p className="px-2 py-3 text-center text-[11px] text-text-mute">{emptyText}</p>
          ) : (
            hits.map((o, i) => (
              <button
                key={String(o.value)}
                ...
                onClick={() => commit(o)}
                className={cn(
                  "flex w-full items-center gap-2 rounded-pix px-1.5 py-1 text-left text-[12px] transition-colors",
        ...
        {q.trim() && hits.length >= limit ? (
          <p className="border-t border-line px-2 py-1 text-[10px] text-text-mute">
